...
const App = () => {
  ...
  const handleLoaderTypeChange = e => {
    setLoaderType(e.target.value);
    setLoaderData("");
  };

  const handleLoaderDataChange = e => {
    setLoaderData(e.target.value);
  };
  ...
  const handleFileChange = e => {
    ...
    // reader.onload = event => {
    //   setLoaderData(event.target.result);
    // };
    // reader.readAsText(file);
  };

  const initializeRAGToolkit = async () => {
    const toolkit = new RAGToolkit();
    try {
      const sources = [{ source: loaderData, type: loaderType }];
      const initializedRetriever = await toolkit.initializeRAG(sources);
      setRetriever(initializedRetriever);
      console.log("RAGToolkit and retriever initialized successfully.");
    } catch (error) {
      console.error("Error initializing RAGToolkit:", error.message);
    }
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
      ...
      <div>
        ...
        ></textarea>
        <div>
          <select value={loaderType} onChange={handleLoaderTypeChange}>
          ...
          {loaderType === "string" && (
            <textarea
              ...
              cols={80}
              value={loaderData}
              onChange={handleLoaderDataChange}
          ...
          <button
            onClick={initializeRAGToolkit}
            style={{ marginBottom: "20px" }}
          >
            Initialize RAGToolkit
          </button>
        </div>
        <div>
          <input
